...
/>
            ...
            <div className="max-w-4xl mx-auto">
                <h1 className="text-center mt-5 text-2xl lg:text-4xl text-amber-500 font-serif font-bold">------Food Items by Category------</h1>
                <div className="flex justify-center mb-4 mt-5">
                    {categories?.map(category => (
                        <button
                            key={category}
                            onClick={() => handleTabClick(category)}
                            className={`px-4 py-2 mx-2 text-sm font-semibold rounded-full focus:outline-none ${activeCategory === category ? 'bg-amber-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                        >
                            {category}
                        </button>
                    ))}
